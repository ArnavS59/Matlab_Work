...
close all
clf
handle_axes= axes('XLim', [-0.4,0.4], 'YLim', [-0.4,0.4], 'ZLim', [-0.4,0.4]);
...
%% Link-0: Base Blue Cylinder
 
trf_link0_E= make_transform([0, 0, -0.05], 0, 0, pi/2, trf_E_axes);
plot_axes(trf_link0_E, 'L_b', false, axis_length); 
 
trf_viz_link0= make_transform([0, 0, 0.1], 0, 0, 0, trf_link0_E);
length0= 0.3; radius0= 0.05;
h(1)= link_cylinder(radius0, length0, trf_viz_link0, [0.01 1 0.8]); 
plot_axes(trf_viz_link0, 'L_g', true, axis_length); % V_0
%% Blue Box
...
%% Joint 0: Aqua Cylinder and Blue Box:Fixed
...
%% Joint 1: Blue Box and Rotating  Cylinder:Rotating
...
%% Joint 2: REd Cyclinder and Orange Cylinder: Fixed
...
length1= 0.05; radius1= 0.04;
h(2)= link_cylinder(radius1, length1, Blackcylinder, [0.75, 0, 0.2]); 
% V_1 and L_1 are the same.
...
h(2)= link_sphere(0.1, Head, [1, 0, 0]); 
%plot_axes(secondarybluebox, ' ', true, axis_length); % V_{1-2}
%% Joint 0: Head and Blue Box:Fixed
Headjoint= make_transform([-0.1, 0, 0], 0, 0, 0, trf_link1p2_link1); 
%plot_axes(Headjoint, 'L_0', false, axis_length); 
make_child(Headjoint, Head);
%% Left Leg
Leftl=make_transform([0.19, 0, -0.3], 0, 0, 0);
h(21)= link_cylinder(0.025,0.15,Leftl,[1 0 1]);
%% Joint leg Fixed
left_leg_joint=make_transform([-0.35, 0, -0.1], 0, -7, 0, trf_link1p2_link1); 
make_child(left_leg_joint,Leftl);
%% Right leg
Rightl=make_transform([0.19, 0, -0.3], 0, 0, 0);
h(32)= link_cylinder(0.025,0.15,Rightl,[1 0 1]);
 %% Joint Right leg Fixed
 Right_leg_joint=make_transform([-0.10, 0, -0.55], 0, -8.8, 0, trf_link1p2_link1); 
 make_child(Right_leg_joint,Rightl);
%% Left eye
left_toe= make_transform([-0.05, -0.05, 0.25], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(10)= link_sphere(0.015, left_toe, [0, 0.5, 1]);
%% Right eye
left_toe= make_transform([-0.05, 0.05, 0.25], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(10)= link_sphere(0.015, left_toe, [0, 0.5, 1]);
%% Smiley
s1= make_transform([-0.08, 0.05, 0.18], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(10)= link_sphere(0.01, s1, [0, 0.5, 1]);

s2= make_transform([-0.08, 0.035, 0.165], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(11)= link_sphere(0.01, s2, [0, 0.5, 1]);


s3= make_transform([-0.08, 0.02, 0.16], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(12)= link_sphere(0.01, s3, [0, 0.5, 1]);

s4= make_transform([-0.08, 0.0, 0.155], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(13)= link_sphere(0.01, s4, [0, 0.5, 1]);

s5= make_transform([-0.08, -0.02, 0.16], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(14)= link_sphere(0.01, s5, [0, 0.5, 1]);

s6= make_transform([-0.08, -0.035, 0.165], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(15)= link_sphere(0.01, s6, [0, 0.5, 1]);


s7= make_transform([-0.08, -0.05, 0.18], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(16)= link_sphere(0.01, s7, [0, 0.5, 1]);
